...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentBody" runat="server">
	...
	<div id="content">
		<div id="DIR_dmclvt_Grid" class="dir" style="position: fixed; width: 100%;"><label class="text-title-grid">Danh mục
			...
			<div style="padding-top: 3px;width: 100%;">
						...
						<tbody>
							<tr>
								<td style="display:inline-flex;line-height:24px;padding-right:8px;cursor:pointer;"
									title="Sửa (Ctrl + E, Alt + E)" class="toolbar-edit " id="TOOLBAR_dmclvt_0_edit">
								...
								<td style="display:inline-flex;line-height:24px;padding-right:8px;cursor:pointer;display:none""
									title="Xem (Alt + W)" class="toolbar-view mobile-hide" id="TOOLBAR_dmclvt_0_view">
				...
				<div class="div-paging col-xs-10 col-sm-10" id="PAGE_dmclvt_0">
					<ul class="pagination pagination-sm grid-page"></ul>
				</div>
			</div>
			<div class="table-bottom">
				<div class="table-scroll-horizontal">
					<table id="TABLE_dmclvt_0" grid-index="0" class="table">
						<colgroup>
							<col style="width: auto;">
							<col class="grid-colgroup ma_clvt" style="width: 120px;">
							<col class="grid-colgroup ten_clvt" style="width: 300px;">
						...
						<thead>
							<tr>
								...
								<th class="grid-column ma_clvt C col-has-focus" style="">
									<div class="btn-header-options"></div>
									<div class="col-header-resize"></div><span>Mã</span>
								</th>
								<th class="grid-column ten_vt C col-has-focus" style="">
									<div class="btn-header-options"></div>
									<div class="col-header-resize"></div><span>Tên</span>
								</th>
						...
						<tbody id="BODY_dmclvt_0" class="data">
						</tbody>
					</table>
				</div>
			</div>
	...
	<div id="subcontent" style="position: fixed; z-index: 1500;">
		...
		<div id="DIR_dmclvt_Dir" class="dir modal" role="dialog" style="z-index: 2002; display: none;">
									...
									<div class="modal-body-header">
										<div class="modal-left-240">
											...
											<div class="form-group">
												<div class="col-xs-24 col-120"><label class="control-label ma_clvt">Mã thương</label></div>
												<div class="col-xs-24 col-120"><input value="" required id="inp_maclvt" spellcheck="false" type="text"  name="ma_clvt"
														class="input-field ma_clvt C AutoIncrease form-control materail-input C"
														style="text-transform: uppercase;" maxlength="32" tabindex="15"></div>
											</div>										
											
											<div class="form-group">
												<div class="col-xs-24 col-120"><label class="control-label ten_clvt">Tên sản
														phẩm</label></div>
												<div class="col-xs-24 col-fill-120"><input id="inp_tenclvt" required
														type="text" class="input-field ten_clvt C  form-control materail-input C"
														style="" maxlength="256" tabindex="16"></div>
											</div>										
											<div class="form-group">
												<div class="col-xs-24 col-120"><label class="control-label ten_clvt2">Tên
														khác</label></div>
												<div class="col-xs-24 col-fill-120"><input id="inp_tenclvt2" value="" spellcheck="false"
														type="text" class="input-field ten_clvt2 C  form-control materail-input C"
		...
		<div id="BACKDROP_Dir_dmclvt_2001" class="modal-backdrop in" style="z-index: 2001;display: none;"></div>
	</div>
	...
	<script>
		...
		$('#TOOLBAR_dmclvt_0_add').click(function() {
			onLoadAction();
			hide('.form-views');
			show('.form-buttons');
			...
			setDisableInputs('#inp_maclvt', false);
			show('#DIR_dmclvt_Dir');
			flag = "NEW";	
		});
		$('#TOOLBAR_dmclvt_0_edit').click(function() {
			onLoadAction();
			setDisableInputs('#inp_maclvt', true);
			setDisableInputs('#inp_tenclvt', true);
			setDisableInputs('#inp_tenclvt2', true);
			setDisableInputs('#inp_mota', true);
			setDisableInputs('#select-status', true);
			setDisableInputs('#inp_thutu', true);
			getInfoDM(key, dataDmClvt);
			show('#DIR_dmclvt_Dir');
			flag = "EDIT";				
		});
		$('#TOOLBAR_dmclvt_0_delete').click(function() {
			flag = "DELETE";		
			...
			$('.btn-ok-confirm').click(function() {	
				if (flag == "DELETE") { 	
					...
					var query = "delete from dmth where ma_clvt = '" + key + "';if exists (select 1 from dmqddvt where ma_clvt = '" + key + "') begin delete from dmqddvt where ma_clvt = '" + key + "' end";
					console.log(query);
					
					var columnkey = "ma_clvt";
					var id = key;
					var action = "DELETE";
					var table = "dbo.dmclvt";
		...
		$('#TOOLBAR_dmclvt_0_view').click(function() {
			onLoadAction();
			...
			$('.btn-delete-form').click(function() { 
				...
				$('.btn-ok-confirm').click(function() {	
					if (flag == "DELETE") { 	
						...
						var query = "delete from dmth where ma_clvt = '" + key + "';if exists (select * from dmqddvt where ma_clvt = '" + key + "') begin delete from dmqddvt where ma_clvt = '" + key + "' end";
						console.log(query);
		...
		$(".btn-zoom").unbind().click(function() {
			...
			$("#DIR_dmclvt_Dir").css("overflowY", "hidden");
		});
		...
		function loadAllData() {
			getDataAll("dmclvt", dataDmClvt, "1");				
		}
		
		function firstLoadPaged() {
			getDm('dmclvt', dataDmClvt);
		}
		...
		function getInfoDM(id, container) {		
			container.forEach(function (item, num) {	
				if (item.ma_clvt == id) {
					console.log(item);	
					$('#inp_maclvt').val(item.ma_clvt);
					$('#inp_tenclvt').val(item.ten_clvt);
					$('#inp_tenclvt2').val(item.ten_clvt2);
		...
		$(".btn-nhan").unbind().click(function() {
			...
			if (flag == "NEW") {				
				...
				var query = "insert into dmth(ma_clvt, ten_vt, ten_vt2, dvt, ton_kho_yn, lo_yn, ma_lvt, nh_vt1, nh_vt2, nh_vt3, ma_kho, ma_vi_tri, sl_min, sl_max, ghi_chu, mo_ta, thong_so, luot_xem, ma_clvt2, ma_km, ma_clvt, nuoc_sx, mau_sac, " + 
							...
							"if not exists (select 1 from dmqddvt where ma_clvt = '" + mavt + "' and dvt = '" + madvt + "') begin insert into dmqddvt select '" + mavt + "', '" + madvt + "', 1, 'S', 1,'" + datetime0 + "', '" + datetime2 + "', '" + userid0 + "', '" + userid2 + "' end";
				console.log(query);
			...
			if (flag == "EDIT") {
				var maclvt = $('#inp_maclvt').val();
				var tenclvt =  $('#inp_tenclvt').val();
				var tenclvt2 = $('#inp_tenclvt2').val();
				var thutu = $('#inp_thutu').val();
				...
				var query = "update dmclvt set ximage1 = N'" + hinhanh1 + "', ximage2 = N'" + hinhanh2 + "' where ma_clvt = '" + key + "';"								
				console.log(query);
			...
			if (text == 'dmclvt') {			
				...
				container.forEach(function (item, num) {		
					html += '<tr>' +
								...
								'<td class="grid-cell ma_clvt C canfocus grid-cell-link" style="" tabindex="1">' + item.ma_clvt + '</td>' +
								'<td class="grid-cell ten_clvt C canfocus" style="" tabindex="2">' + item.ten_clvt + '</td>' +
